...
public class BabysitterApp {
	...
	do {	
		...
		if(userInput == 1) {
			...
			System.out.println("\nWhat was the start time for this job?");
			...
			int userInput4 = input.nextInt();
				...
				while(!isValidEnd(userInput4)) {
					...
					System.out.println("End time must be an hour between 6pm and 4am (no shift may go past 4am). Enter as a whole number "
							+ "without \"am\" or \"pm\".");
					System.out.println("Example: 7\n");
					
					System.out.println("What was the end time for this job?");
					userInput3 = input.nextInt();
				}
